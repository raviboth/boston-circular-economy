...
Cursor is an AI-first code editor with built-in AI capabilities. It supports SKILL.md skills and can discover them from multiple locations. Cursor does not have a native `AskUserQuestion` tool, so questions are presented as plain text with numbered options.

**Key capabilities:**
- SKILL.md skill discovery from multiple directories
- Extended skill structure with `scripts/`, `references/`, and `assets/` subdirectories
- Progressive resource loading for efficient context usage
...
**Note:** Cursor also discovers skills from `~/.claude/skills/` and `.codex/skills/`. If you already have skills installed at `~/.claude/skills/`, Cursor will find them automatically.

## Question Catalog

This section defines every customization question for Cursor, with plain-text presentation examples. Since Cursor does not have `AskUserQuestion`, present each question as text with numbered options and ask the user to choose.
...
**Detection logic:** Check `atelier.commands.plan` and `atelier.commands.implement` for the string `cursor`. If found, auto-detect and confirm.

**Settings affected:** Determines which guide to load.

#### When auto-detected:

> I detected that you're using **Cursor** based on your current command settings. Is that correct?
>
> 1. **Yes, that's correct** — Continue with Cursor configuration
> 2. **Claude Code** — Switch to Anthropic's Claude Code CLI
> 3. **OpenAI Codex** — Switch to OpenAI Codex CLI
> 4. **GitHub Copilot** — Switch to GitHub Copilot CLI
...
**Cursor has no agent-specific features to configure.** Skip this question for Cursor.

Agent Teams is not supported by Cursor.
...
>
> 1. **User directory (Recommended)** — `~/.cursor/skills` — available across all workspaces
> 2. **Project directory** — `.cursor/skills` — scoped to this repo, committed with code
> 3. **Custom directory** — specify a different path
>
> Cursor also discovers skills from `~/.claude/skills/`, so if you already have skills there, they'll be found automatically.

**If "Custom directory" is chosen**, ask:

> Enter the path for your skills directory (or choose a suggestion):
>
> 1. `~/.cursor/skills` — standard Cursor location
> 2. `.cursor/skills` — project-level directory
...
**How to construct the suggestion:** Use the Cursor CLI with the `{{prompt}}` placeholder.

Example for `plan`:

> What command should run for planning?
>
> 1. **Use suggested command** — `cursor "{{prompt}}"`
...
|--------|------------------|
| plan | `cursor "{{prompt}}"` |
| implement | `cursor "{{prompt}}"` |
| verifyTask | `cursor "{{prompt}}"` |
| verifyPlan | `cursor "{{prompt}}"` |
| verifyManual | `cursor "{{prompt}}"` |
| brainstorm | `cursor "{{prompt}}"` |
...
Cursor does not have native `TaskCreate`/`TaskUpdate` tools. The atelier-plan skill automatically detects this and writes task JSON files directly to the `taskDir` path provided in the prompt. No additional configuration is needed.
...
Not supported by Cursor at this time.
...
> The recommended command template now includes a new flag. Update your plan command?
>
> 1. **Update command** — applies the recommended command template
> 2. **Keep current** — no change
...
- **Extended skill structure**: Cursor supports `scripts/`, `references/`, and `assets/` subdirectories within skills for executable code, supplementary docs, and templates
- **Progressive resource loading**: Skills load resources on demand, keeping context usage efficient
- **Migration tool**: Use the `/migrate-to-skills` command to convert existing Cursor Rules and dynamic rules to skill format
- **Import from GitHub**: Import skills directly from GitHub repositories via Settings > Rules > Add Rule > Remote Rule
- **Disable auto-invocation**: Set `disable-model-invocation: true` in SKILL.md frontmatter to require explicit `/skill-name` invocation
