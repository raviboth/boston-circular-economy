...
GitHub Copilot provides AI-assisted coding through the CLI and VS Code. It supports SKILL.md skills and can discover them from multiple locations. Copilot does not have a native `AskUserQuestion` tool, so questions are presented as plain text with numbered options.

**Key capabilities:**
- SKILL.md skill discovery from multiple directories
- Repository-level skill sharing via `.github/skills/`
- Pre-built skills available in the `github/awesome-copilot` collection
...
.github/skills
```
Skills installed here are scoped to this repository and can be committed with the code. This is the standard GitHub convention for repository-level configuration.

**Note:** Copilot also discovers skills from `~/.claude/skills/`. If you already have skills installed at `~/.claude/skills/`, Copilot will find them automatically.

## Question Catalog

This section defines every customization question for GitHub Copilot, with plain-text presentation examples. Since Copilot does not have `AskUserQuestion`, present each question as text with numbered options and ask the user to choose.
...
**Detection logic:** Check `atelier.commands.plan` and `atelier.commands.implement` for the string `copilot`. If found, auto-detect and confirm.

**Settings affected:** Determines which guide to load.

#### When auto-detected:

> I detected that you're using **GitHub Copilot** based on your current command settings. Is that correct?
>
> 1. **Yes, that's correct** — Continue with GitHub Copilot configuration
...
**GitHub Copilot has no agent-specific features to configure.** Skip this question for Copilot.

Agent Teams is not supported by GitHub Copilot.
...
>
> 1. **User directory (Recommended)** — `~/.copilot/skills` — available across all workspaces
> 2. **Project directory** — `.github/skills` — scoped to this repo, committed with code (standard GitHub convention)
> 3. **Custom directory** — specify a different path
>
> Copilot also discovers skills from `~/.claude/skills/`, so if you already have skills there, they'll be found automatically.

**If "Custom directory" is chosen**, ask:

> Enter the path for your skills directory (or choose a suggestion):
>
> 1. `~/.copilot/skills` — standard Copilot location
> 2. `.github/skills` — project-level directory
...
**How to construct the suggestion:** Use the Copilot CLI with the `{{prompt}}` placeholder.

Example for `plan`:

> What command should run for planning?
>
> 1. **Use suggested command** — `copilot "{{prompt}}"`
...
|--------|------------------|
| plan | `copilot "{{prompt}}"` |
| implement | `copilot "{{prompt}}"` |
| verifyTask | `copilot "{{prompt}}"` |
| verifyPlan | `copilot "{{prompt}}"` |
| verifyManual | `copilot "{{prompt}}"` |
| brainstorm | `copilot "{{prompt}}"` |
...
Copilot does not have native `TaskCreate`/`TaskUpdate` tools. The atelier-plan skill automatically detects this and writes task JSON files directly to the `taskDir` path provided in the prompt. No additional configuration is needed.
...
Not supported by GitHub Copilot at this time.
...
- The Copilot coding agent is available in VS Code Insiders with stable support coming soon
- Skills can also be placed in `.github/skills/` for repository-level sharing
- Pre-built skills are available in the `github/awesome-copilot` collection
